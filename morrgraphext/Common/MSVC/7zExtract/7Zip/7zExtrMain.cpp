...
SRes SzFileReadImp(void *object, void **buffer, size_t *size)
{
 static BYTE g_Buffer[kBufferSize];
  ...
  if (*size > kBufferSize)
    *size = kBufferSize;
  *size = Ex7zReadFile(s->File, g_Buffer, *size);
  *buffer = g_Buffer;
  return SZ_OK;
}
...
{
 CMemoryInStream *s = (CMemoryInStream *)object;
 static BYTE g_Buffer[kBufferSize];

 if (*size > kBufferSize)*size = kBufferSize;
 CopyMemory(&g_Buffer, &((char*)s->BaseAddress)[s->CurrentPos], *size);
 s->CurrentPos += *size;
 *buffer = g_Buffer;
 return SZ_OK;
}
...
SRes SzMemSeekImp(void *object, CFileSize pos, ESzSeek origin)
{
  ...
  switch (origin) 
   {
    case SZ_SEEK_SET: s->CurrentPos  = pos; break;
    case SZ_SEEK_CUR: s->CurrentPos += pos; break;
