...
public:
    ...
    static ULONG CoMemCopyWFEX(const WAVEFORMATEX * pSrc, WAVEFORMATEX ** ppCoMemWFEX)
    {
                             ...
                             CComPtr<ISpPhoneConverter> cpPhoneConv = NULL)
{
    ...
    //SPPHONEID* pphoneId = dsPhoneId;      // !!! WRONG !!!
    ...
    for (i = 0; SUCCEEDED(hr) && i < cWords; i++)
    {
        ...
        if (*p == L'/')
        {
            ...
            if ( pszThirdPart)
            {
                hr = cpPhoneConv->PhoneToId(pszThirdPart, NULL);
                if (SUCCEEDED(hr))
                {
                    pPhraseElement[i].pszPronunciation = NULL;
                   // pphoneId += wcslen(NULL) + 1;
                }
            }
        }
        else
