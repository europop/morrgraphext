...
void TiXmlBase::PutString( const TIXML_STRING& str, TIXML_STRING* outString )
{
	...
	{
		int c = (unsigned char)str[i];   //  	int c = str[i]; - only positive
		...
		else if ( c < 32) /* TODO :  || c > 126 ) - Commented to support russian symbols */
		{
...
const char * TiXmlElement::Attribute( const char * name, int* i ) const
{
	...
	{
		if ( s )
			*i = strtol(s, NULL, 0); //   atoi( s ); // atoi not supports 0xHEX
		else
			*i = 0;
	}
	return s;
...
bool TiXmlDocument::Assign( const char* XmlBuffer )
{
 TIXML_STRING data;
