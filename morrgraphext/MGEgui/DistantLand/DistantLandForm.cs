...
namespace MGEgui.DistantLand {
	...
	// set of keys to write after plugin selection
        ...
        private void SaveStatSettings() {
            ...
            iniFile.setKey("LowQualTex", cbStatLowQualTextures.Checked);
            iniFile.setKey("Activators", cbStatActivators.Checked);
            iniFile.setKey("MiscObj", cbStatIncludeMisc.Checked);
            iniFile.setKey("OldSimply", cbStatOldSimplification.Checked);
        ...
        void workerCreateStatics(object sender, System.ComponentModel.DoWorkEventArgs e) {
            ...
            if (args.MipSkip != 0 && cbStatLowQualTextures.Checked) {
                ...
                br.Close();
                stc.Dispose();
                if (archive.GetItemsCount() > 0) archive.Save(Statics.fn_stattexbsa);
                archive.Clear();
            }
        ...
        void workerFCreateStatics(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e) {
            ...
            foreach (String statfile in new String[] { Statics.fn_usagedata, Statics.fn_statmesh, Statics.fn_stattexbsa }) {
                if (File.Exists(statfile)) {
                    fileSize += (new FileInfo(statfile)).Length;
                    filecount++;
                }
            }
        ...
        void workerExportStatics(object sender, System.ComponentModel.DoWorkEventArgs e) {
            ...
            unsafe {
                NativeMethods.BeginStaticCreation((IntPtr)DXMain.device.UnmanagedComPointer, null);
            }
        ...
        private class MWPlugin {
            ...
            public MWPlugin(FileInfo file, bool ESM, CheckState state) {
                ShortName = file.Name;
                Name = file.Name;
                FullName = file.FullName.ToLower(Statics.Culture);
                this.ESM = ESM;
            ...
            public MWPlugin(FileInfo file, string dir, bool ESM, CheckState state) {
                ShortName = file.Name;
                Name = file.Name + " > " + dir;
                FullName = file.FullName.ToLower(Statics.Culture);
                Dir = dir;
        ...
        private class MWPlugins {
            ...
            public MWPlugins(String main, List<String> dirs, String[] wizardplugins) {
                DirectoryInfo dir = new DirectoryInfo(main);
                datafiles = dir.FullName.ToLower(Statics.Culture);
                List<String> presets = new List<String>();
                for (int i = 0; i < wizardplugins.Length; i++) presets.Add(wizardplugins[i].ToLower(Statics.Culture));
                for (int i = 0; i < dirs.Count; i++) if (String.Compare(Path.GetFullPath(dirs[i]), Path.GetFullPath(Statics.fn_datafiles), true, Statics.Culture) == 0) dirs.RemoveAt(i--);
                FileInfo[] files;
                Plugins = new Dictionary<string, MWPlugin>();
                Dirs = new List<string>(dirs);
                files = dir.GetFiles("*.esm");  //  For compatibility with WINE
                //files = dir.GetFiles("*?.esm");
                foreach (FileInfo file in files) Plugins.Add(file.Name.ToLower(Statics.Culture),
                    new MWPlugin(file, true, presets.Contains(file.Name.ToLower(Statics.Culture)) ? CheckState.Checked : presets.Count > 0 ? CheckState.Unchecked : CheckState.Indeterminate));
                files = dir.GetFiles("*.esp");  //  For compatibility with WINE
                //files = dir.GetFiles("*?.esp");
                foreach (FileInfo file in files) Plugins.Add(file.Name.ToLower(Statics.Culture),
                    new MWPlugin(file, false, presets.Contains(file.Name.ToLower(Statics.Culture)) ? CheckState.Checked : presets.Count > 0 ? CheckState.Unchecked : CheckState.Indeterminate));
                List<string> removeDirs = new List<string>();
                foreach (string dirName in Dirs) {
                    dir = new DirectoryInfo(dirName);
                    if (dir.Exists) {
                        CheckState state = String.Compare(dir.FullName, Path.GetFullPath(Statics.fn_statics), true, Statics.Culture) == 0 ? CheckState.Indeterminate : CheckState.Unchecked;
                        files = dir.GetFiles("*.esm");  //  For compatibility with WINE
                        //files = dir.GetFiles("*?.esm");
                        foreach (FileInfo file in files) Plugins.Add((file.Name + " > " + dirName).ToLower(Statics.Culture),
                            new MWPlugin(file, dirName, true, presets.Contains((file.Name + " > " + dirName).ToLower(Statics.Culture)) ? CheckState.Checked : state));
                        files = dir.GetFiles("*.esp");  //  For compatibility with WINE
                        //files = dir.GetFiles("*?.esp");
                        foreach (FileInfo file in files) Plugins.Add((file.Name + " > " + dirName).ToLower(Statics.Culture),
                            new MWPlugin(file, dirName, false, presets.Contains((file.Name + " > " + dirName).ToLower(Statics.Culture)) ? CheckState.Checked : state));
            ...
            public void SetDirs(List<string> dirs) {
                ...
                foreach (string dirName in dirs) if (Dirs.IndexOf(dirName) == -1) {
                    ...
                    if (dir.Exists) {
                        ...
                        files = dir.GetFiles("*.esm"); //  For compatibility with WINE
                        //files = dir.GetFiles("*?.esm");
                        foreach (FileInfo file in files) Plugins.Add((file.Name + " > " + dirName).ToLower(Statics.Culture), new MWPlugin(file, dirName, true, state));
                        files = dir.GetFiles("*.esp"); //  For compatibility with WINE
                        //files = dir.GetFiles("*?.esp");
                        foreach (FileInfo file in files) Plugins.Add((file.Name + " > " + dirName).ToLower(Statics.Culture), new MWPlugin(file, dirName, false, state));
                    } else removeDirs.Add(dirName);
                }
                foreach (string dirName in removeDirs) dirs.Remove(dirName);
                Dirs.Clear();
        ...
        private void bPlugsRun_Click(object sender, EventArgs e) {
            ...
            i = 0;
            foreach (KeyValuePair<string, MWPlugin> item in list) if (item.Value.Checked != CheckState.Unchecked) {
