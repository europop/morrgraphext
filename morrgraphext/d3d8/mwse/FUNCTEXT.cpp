...
bool FUNCSTRINGMATCH::execute(void)
{
	...
	if ( machine.pop(regString) && machine.pop(regPattern) ) {
		...
		if (string != NULL && pattern != NULL) {
			// TODO: Implement partial match regex_search without using boost library
			//match = boost::regex_search(string, boost::regex(pattern), boost::match_partial); // boost::match_default = 0
		}
	}
